...
public class LibraryRepository {
    ...
    static {
        bookList = new Book[]{
                ...
                new CartoonBook("떨어지면 끝장", "스에노부 케이코", "대원싸아이", 18),
                new CartoonBook("원펀맨", "원", "대원싸아이", 15),
                new CartoonBook("이세계로 전이했으니 치트를 살려 마법검사가 되기로 했다", "shinkokiro", "대원싸아이", 18),
    ...
    // 저자이름을 받으면 해당 저자의 책을 반환

    public String[] searchBookByAuthor(String AuthorName) {
        StringList authorBookList = new StringList();
        for (Book author : bookList) {
            String authors = author.getAuthor();

            if (AuthorName.equals(authors)) {
                String bookFound = author.info();
                authorBookList.push(bookFound);
            }


        }
        return authorBookList.getsArr();
    }
